...
class MpesaC2BEvent(Base):
    ...
    trans_id: Mapped[str] = mapped_column(
        String(50),
        nullable=False,
        unique=True,
        index=True,
    )
    business_short_code: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        index=True,
    )
    bill_ref_number: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
        index=True,
    )
    derived_student_number: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        index=True,
    )
    ...
    # Some providers send long identifiers here, so allow up to 128 chars.
    msisdn: Mapped[str | None] = mapped_column(
        String(128),
        nullable=True,
        index=True,
    )
    ...
    status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default=MpesaC2BEventStatus.RECEIVED.value,
        index=True,
    )
