...
- `POST /billing-accounts/{account_id}/refunds/preview` — preview account-level refund: refundable total, free credit, amount to reopen, affected invoices and payment source attribution. Optional `invoice_reversals: [{ invoice_id, amount }]` lets UI choose refund sources by invoice; backend resolves them into allocation reversals. Legacy `allocation_reversals: [{ allocation_id, amount }]` is still supported for technical callers.
...
- `POST /billing-accounts/{account_id}/refunds` — создать account-level refund document. Требует amount, refund_date, reason и proof: `reference_number`, `proof_text` или `proof_attachment_id`. По умолчанию free credit используется первым, затем allocation reversals newest-first. Optional `invoice_reversals` must exactly equal `amount_to_reopen` and lets admins choose which invoices are reopened; backend splits each invoice amount across its allocations.
...
- `POST /payments/allocations/auto` — body принимает `student_id` или `billing_account_id`; порядок: activity invoices first независимо от term, затем previous-term debt, active/current term и future/no-term invoices; внутри одного bucket `requires_full` invoices идут перед proportional partial allocation
