...
- React + TypeScript
- Vite
- Tailwind CSS + кастомные UI компоненты (без MUI)
- React Router
...
**Интерфейс бухгалтера (Accountant):** создайте пользователя с ролью Accountant (Settings → Users, только SuperAdmin). Войдите под этим пользователем — откроется дашборд с минимальным меню: Dashboard, Documents (Incoming Payments, Students Invoices, PO, GRN, Procurement Payments, Employee Expenses Claims, Employee Payouts), Data Export, Audit Trail. Бухгалтер видит общий реестр student payments на странице `Billing → Payments` / `/billing/payments`, список платных активностей на `Billing → Activities` / `/billing/activities` и billing accounts на `Billing → Billing Accounts` / `/billing/families`, включая roster, shared balance и statement. Новый admission создаётся одним flow на `/students/new`: сначала заводится billing contact / payer account, затем добавляется один или несколько детей и при необходимости линкуются existing students. Создание/редактирование billing accounts и ручной приём платежей остаются доступны только `SuperAdmin` / `Admin`. При ручном приёме student или billing account payment админ может выбрать конкретный invoice, чтобы сумма сначала аллоцировалась туда, а не в старые долги.
...
| Роль | Возможности |
|------|-------------|
| SuperAdmin | Полный доступ: Users/Settings, Terms/Pricing, Catalog, approve/reject/send-to-edit claims, approve GRN, cancel payments, bank reconciliation |
...
| User | Сотрудник: создаёт и видит только свои expense claims; может редактировать и повторно отправлять claim после `send-to-edit`; остальное по минимуму |
