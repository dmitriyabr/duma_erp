...
- [ ] **Производительность:** убрать N+1 на странице студентов (balance + debt на каждого студента), на PayoutsPage (balance на каждого сотрудника), на ReservationsPage (500 студентов для имён). Batch-эндпоинты или расширенные ответы списков.
...
- [ ] **Поиск:** debounce для полей поиска (Students, Users, Items, Stock, Movements, Catalog).
- [ ] **Типы:** общие ApiResponse/PaginatedResponse в одном модуле; привести форму Add invoice line к контракту API (только kit_id).
- [ ] **UX:** индикаторы загрузки в таблицах; Error Boundary; пагинация там, где списки большие.
- [ ] **Мелкое:** удалить ProcurementPaymentsListPage.tsx.bak; константы лимитов; хелперы прав по ролям.
