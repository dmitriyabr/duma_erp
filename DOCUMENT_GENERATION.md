...
│  Student: John Doe                    Academic Year: ...    │
│  Admission #: 2024-0123                    CLASS: Grade 7   │
│  Parent: Mary Doe                                            │
...
**Bill To Section:**
- Левая колонка: Student, Admission #, Parent, Phone
- Правая колонка: TERM, Academic Year, **CLASS**
...
**Footer:**
- Page number и generation date/time (Notes на счёте не выводятся; выписка по балансу — отдельная задача)
...
**Размер страницы:** A4 (210mm x 297mm). Макет рассчитан на умещение на одну страницу (компактные отступы и шрифт).

**Margins:**
- 12mm со всех сторон (уменьшены для одной страницы)
...
**Концепция:** Receipt = электронный чек о получении платежа. Баланс студента и выписка по нему — отдельная задача (Statement).
...
├─────────────────────────────────────────────────────────────┤
│  RECEIVED FROM:                                              │
│  Student: John Doe          │  Class: Grade 7               │
│  Admission #: 2024-0123      │  Parent: Mary Doe             │
│                             │  Phone: +254 712 345 678      │
├─────────────────────────────────────────────────────────────┤
│  PAYMENT DETAILS:                                            │
│  Payment Method: M-Pesa                                      │
│  Transaction ID: ABC123456789 (if any)                       │
│                                                              │
│  ┌───────────────────────────────────────────────────────┐  │
...
- Две колонки: слева — Student, Admission #; справа — Class, Parent, Phone
- Border вокруг

**Payment Details:**
- Payment method (M-Pesa / Bank / Cash / Cheque)
- Transaction ID/Reference (если есть)
- **Amount Received** — в рамке, крупным шрифтом, жирным
- Amount in words — для юридической валидности

**Stamp/Seal (опционально):**
- Изображение печати школы (справа под блоком Amount Received). Блок «Received by» с подписью в электронном чеке не выводится.
...
class PDFService:
    ...
    def generate_receipt_pdf(self, payment: PaymentDetail) -> bytes:
        ...
        Receipt shows payment received (student, amount, method, reference).
        Balance/statement is a separate feature.
        Payment object should have:
        - amount, receipt_number, payment_method, reference
...
<body>
    ...
    {% if stamp_data_uri %}
    <div class="stamp-wrap"><img src="{{ stamp_data_uri }}" alt="School Stamp" class="stamp"></div>
    {% endif %}
