# План фичи: Family Billing / общий семейный платежный аккаунт

## 1. Цель

Нужен механизм, который позволит:
- объединять нескольких учеников в одну семью;
- принимать оплату на один общий billing account;
- автоматически распределять деньги по invoice всех детей в семье;
...
Вывод: для family billing нужна отдельная сущность-владелец денег.
...
Для UX можно использовать слово `Family`.

Для архитектуры лучше мыслить сущностью `BillingAccount`.

Рекомендация:
- в базе и backend: `BillingAccount`;
- в интерфейсе: `Family` или `Family Account`, если это понятнее пользователям.
...
- auto-allocation видит invoice всех активных членов семьи;
- остаток денег остается как общий family credit.

### 5.2. Invoice остается student-owned

Даже после внедрения family billing:
- invoice создается на конкретного ученика;
- в отчетах видно, какой именно ребенок должен;
- в family view просто агрегируются invoice всех членов семьи.
