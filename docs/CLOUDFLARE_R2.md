...
R2 — S3-совместимое хранилище Cloudflare. Используется для **attachments** (подтверждения, proof, bank statement CSV imports и т.д.). Для bulk‑CSV сценариев типа inventory/PO файл обычно парсится и не сохраняется, но bank statements сохраняются как Attachment, чтобы их можно было скачать.
